...
@('zz_install_frostbite.ps1', 'install_frostbite.bat', 'UNWISE.bat', 'zz_install_frostbite.ps1') | ForEach-Object {
    $batchSource = Join-Path $scriptdir $_
    if (Test-Path $batchSource) {
        try {
            $batchDest = Join-Path $installDir $_
            Copy-Item -Path $batchSource -Destination $batchDest -Force
            $copiedList += (Resolve-Path -LiteralPath $batchDest).Path
            Write-Host "Installed batch wrapper: ${_}" -ForegroundColor Green
        } catch { Write-Warning "Failed to copy batch wrapper ${_}: $_" }
    }
}
...
foreach ($f in $filesToHide) {
    ...
    if (Test-Path $target) {
        try {
            ...
            # Use bitwise OR to add the Hidden attribute without removing Archive/System attributes
            $item.Attributes = $item.Attributes -bor [System.IO.FileAttributes]::Hidden
            Write-Host "Hid internal script: $f" -ForegroundColor Gray
        } catch { 
            Write-Warning "Failed to hide ${f}: $_" 
        }
    }
}
...
function Update-And-RegisterTask {
    ...
    try {
        if ($schtask.Task.RegistrationInfo) {
            if ($schtask.Task.RegistrationInfo.Date) { $schtask.Task.RegistrationInfo.Date = $timestamp }
            if ($schtask.Task.RegistrationInfo.Author) { $schtask.Task.RegistrationInfo.Author = $user }
        }
